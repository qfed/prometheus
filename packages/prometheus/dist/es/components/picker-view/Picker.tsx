...
import BaseCompenent from '../../base/baseComponent'
...
class Picker extends BaseCompenent<IPickerProp & IPickerProps, any> {
  static defaultProps = {
    is3D: false,
  }
  ...
  scrollHanders = (() => {
    ...
    const scrollTo = (_x, y, time = 0.3) => {
      if (scrollY !== y) {
        ...
        if (this.props.is3D) {
          const itemList = this.contentRef.querySelectorAll('.pro-picker-item')
          const currentIdx = y / this.itemHeight
          for (let i = 0; i < itemList.length; i++) {
            itemList[i].style.transform = `rotateX(${(currentIdx - i) *
              25}deg) scale(${1 - Math.abs(currentIdx - i) * 0.1})`
          }
        }
        setTimeout(() => {
    ...
    const onMove = (y: number) => {
      ...
      if (this.props.is3D) {
        const itemList = this.contentRef.querySelectorAll('.pro-picker-item')
        const currentIdx = scrollY / this.itemHeight
        for (let i = 0; i < itemList.length; i++) {
          itemList[i].style.transform = `rotateX(${(currentIdx - i) *
            25}deg) scale(${1 - Math.abs(currentIdx - i) * 0.1})`
        }
  ...
  render() {
    ...
    const prefixCls = `${this.classPrefix}picker`
